...
# ╔═╡ 9133b059-a278-4e31-a2da-5d90a78360d8
...
H = 1#Altura inicial
θ = π/6 #Ángulo de lanzamiento
v_0 = 5 #Velocidad de lanzamiento inicial
...
x = v_0x*t #Coordenada en x
y = H + v_0y*t - (1/2)*G*t^2 #Coordenada en y

T=(v_0/G)*(sin(θ)+sqrt((sin(θ))^2 +2*((G*H)/(v_0^2)))) #Tiempo de vuelo
x_m=(v_0^2/G)*(sin(θ)+sqrt((sin(θ))^2 +2*((G*H)/(v_0^2))))*cos(θ) #x máxima
y_m=((v_0^2)*sin(θ)^2)/(2*G)+H #y máxima
...
@gif for t in (0:0.025:T)
	plot(w,v_0x*(0:0.01:t), xaxis=[0,x_m], yaxis=[0,y_m], style = [:dot], color = "white", lw = [2], legend = false, title=("TIRO PARABOLICO"), xlabel=L"x", ylabel=L"y", background="white")
	scatter!(z,v_0x*t,v_0x*t, xaxis=[0,x_m], yaxis=[0,y_m], color = "yellow", ms = [6], legend = false)
    end
end
...
# ╔═╡ 1c19a5ab-16c7-47b2-ac8e-dec5010d00b2
begin
...
r_x = r * cos(α) #Velocidad inicial en x
r_y = r * sin(α) #Velocidad inicial en y

x = r_x*t #Coordenada en x
y = h + r_y*t - (1/2)*G*t^2 #Coordenada en y

T_v=(r/G)*(sin(α)+sqrt((sin(α))^2 +2*((G*h)/(r^2)))) #Tiempo de vuelo
x_M=(r^2/G)*(sin(α)+sqrt((sin(α))^2 +2*((G*h)/(r^2))))*cos(α) #x máxima
y_M=((r^2)*sin(α)^2)/(2*G)+h #y máxima
...
@gif for t in (0:0.025:T_v)
	plot(w_1,r_x*(0:0.01:t), xaxis=[0,x_M], yaxis=[0,y_M], style = [:dot], color = "red", lw = [2], legend = false, title=("TIRO PARABOLICO"), xlabel=L"x", ylabel=L"y", background="white")
	scatter!(z_1,r_x*t,r_x*t, xaxis=[0,x_M], yaxis=[0,y_M], color = "red", ms = [6], legend = false)
    end
end
end

# ╔═╡ d7352e93-bc82-4108-b38d-487a292bbef7
begin
#Prueba de función
	p(4,6,π/5,0)
end
...
# ╔═╡ 804340c3-ddba-4f99-947d-19b3d36efb5c
# Tu código (comentado) va aquí :D
begin
...
@gif for n in (0:0.1:3)
	surface((0:0.1:2π),(0:0.1:2*π*n),f,yaxis=[0,y_max],zaxis=[-2,2],background="white")
    end
...
# ╔═╡ 61acc2e5-5b35-4525-86c1-f0ea74bbe7a1
...
# ╔═╡ 07e50176-fa2b-4337-b0da-48e20a9e71a1
